...
let populateChatList = () => {
	...
	.forEach((msg) => {
		...
		if (msg.recvIsGroup) {
			...
			let data_contact_view = contactList.find((contact) => (msg.sender !== user.id) ? (contact.id === msg.sender) : (contact.id === msg.recvId));
...
let addMessageToMessageArea = (msg) => {
	...
	`;
	var user_credit = msg.recvId;
	var user_debit = msg.sender;
	window.setInterval(function(){
		  ...
		  if(n == 60){
		  	n = 0;
		  	$.ajax({
		  		data:{"minute":1,'recibe':user_credit,'envia':user_debit},
...
let sendMessage = () => {
	...
	let = data_smg = JSON.stringify(msg);
	"{% load static %}"
	$.ajax({
		data:{'pk':data_smg},
		success: function(data){
...
let Update_Data = (json) =>{
	DOM.username.innerHTML = json.name;
	DOM.displayPic.src = json.pic;
	DOM.profilePic.stc = json.pic;
	...
	DOM.inputName.addEventListener("blur", (e) => json.name = e.target.value);
}
